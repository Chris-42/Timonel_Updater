...
std::unordered_set<uint8_t> active_addresses;
...
NbMicro::NbMicro(uint8_t twi_address, uint8_t sda, uint8_t scl) : addr_(twi_address), sda_(sda), scl_(scl) {
    ...
    if (!((sda_ == 0) && (scl_ == 0))) {
#if ((defined DEBUG_LEVEL) && (DEBUG_LEVEL >= 1))
        USE_SERIAL.printf_P("[%s] NBMicro instance created with address %d -> [SDA pin: %d] [SCL pin: %d]\n\r", __func__, addr_, sda_, scl_);
#endif                           // DEBUG_LEVEL
        Wire.begin(sda_, scl_);  // Init I2C sda_:GPIO0, scl_:GPIO2 (ESP-01) / sda_:D3, scl_:D4 (NodeMCU)
    } else {
#if ((defined DEBUG_LEVEL) && (DEBUG_LEVEL >= 1))
        USE_SERIAL.printf_P("[%s] NBMicro instance created with address %d -> [SDA pin: %d] [SCL pin: %d]\n\r", __func__, addr_, SDA_STD_PIN, SCL_STD_PIN);
#endif                                         // DEBUG_LEVEL
...
NbMicro::NbMicro(uint8_t twi_address, TwoWire &Wire) : addr_(twi_address), sda_(0), scl_(0) {
#if (ARDUINO_ARCH_ESP8266 || ARDUINO_ESP32_DEV || ESP_PLATFORM)
    // If the TWI address is not in use, create a new NbMicro object (ESP micros)
    if ((!active_addresses.insert(addr_).second) && (addr_ != 0)) {
#if ((defined DEBUG_LEVEL) && (DEBUG_LEVEL >= 1))
        USE_SERIAL.printf_P("[%s] Error: The TWI address [%02X] is in use! Unable to create another device object with it ...\r\n", __func__, addr_);
        USE_SERIAL.printf_P("[%s] Execution terminated, please review the devices' TWI addresses on your code.\r\n", __func__);
#endif  // DEBUG_LEVEL
        delay(DLY_NBMICRO);
        std::terminate();
    ...
    // If SDA and SCL pins are not specified, use the default ones
#if ((defined DEBUG_LEVEL) && (DEBUG_LEVEL >= 1))
    USE_SERIAL.printf_P("[%s] NBMicro instance created with address %02X with provided Wire\n\r", __func__, addr_);
#endif                                         // DEBUG_LEVEL
#else   // -----
    // If the TWI address is not in use, create a new NbMicro object (AVR micros)
    for (uint8_t i = 0; i < TWI_DEVICE_QTY; i++) {
...
NbMicro::~NbMicro() {
#if ((defined DEBUG_LEVEL) && (DEBUG_LEVEL >= 1))
    USE_SERIAL.printf_P("[%s] Removing NBMicro instance with TWI address %02X ...\r\n", __func__, addr_);
#endif  // DEBUG_LEVEL
#if (ARDUINO_ARCH_ESP8266 || ARDUINO_ESP32_DEV || ESP_PLATFORM)
...
uint8_t NbMicro::SetTwiAddress(uint8_t twi_address) {
    if (addr_ != 0) {
#if ((defined DEBUG_LEVEL) && (DEBUG_LEVEL >= 1))
        USE_SERIAL.printf_P("[%s] Object TWI address already defined, using 0x%02X\r\n", __func__, addr_);
#endif  // DEBUG_LEVEL
        return ERR_ADDR_IN_USE;
    } else {
        addr_ = twi_address;
#if ((defined DEBUG_LEVEL) && (DEBUG_LEVEL >= 1))
        USE_SERIAL.printf_P("[%s] Object TWI address correctly set to %0x%02X\r\n", __func__, addr_);
#endif  // DEBUG_LEVEL
        return 0;
    }
}
...
#if ((defined DEBUG_LEVEL) && (DEBUG_LEVEL >= 1))
    USE_SERIAL.printf_P("[%s] Initializing Micro %02X ...\r\n", __func__, addr_);
#endif  // DEBUG_LEVEL
