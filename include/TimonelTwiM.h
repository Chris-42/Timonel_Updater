...
class Timonel : public NbMicro {
   public:
    ...
    Timonel(const uint8_t twi_address, TwoWire &Wire);
    ~Timonel();
    ...
    DevSettings GetDevSettings(void);
      ...
      //void setProgressCallback(void (*)(uint8_t));
      void setProgressCallback(void (*cb)(uint8_t));
   private:
