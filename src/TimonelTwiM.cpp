...
Timonel::Timonel(const uint8_t twi_address, const uint8_t sda, const uint8_t scl) : NbMicro(twi_address, sda, scl) {
    if ((addr_ >= LOW_TML_ADDR) && (addr_ <= HIG_TML_ADDR)) {
        ...
        USE_SERIAL.printf_P("[%s] Bootloader instance created with TWI address 0x%02X.\r\n", __func__, addr_);
#endif /* DEBUG_LEVEL */
        BootloaderInit();
    } else {
#if ((defined DEBUG_LEVEL) && (DEBUG_LEVEL >= 1))
...
Timonel::Timonel(const uint8_t twi_address, TwoWire &Wire) : NbMicro(twi_address, Wire) {
    if ((addr_ >= LOW_TML_ADDR) && (addr_ <= HIG_TML_ADDR)) {
#if ((defined DEBUG_LEVEL) && (DEBUG_LEVEL >= 1))
        USE_SERIAL.printf_P("[%s] Bootloader instance created with TWI address 0x%02X.\r\n", __func__, addr_);
#endif /* DEBUG_LEVEL */
...
Timonel::~Timonel() {
#if ((defined DEBUG_LEVEL) && (DEBUG_LEVEL >= 1))
    USE_SERIAL.printf_P("[%s] Removing bootloader instance with TWI address 0x%02X ...\r\n", __func__, addr_);
#endif  // DEBUG_LEVEL
}
...
/* _________________________
  ...
  |_________________________|
*/
// set a callback for the flash progress
void Timonel::setProgressCallback(void (*cb)(uint8_t)) {
  progressCbPtr = cb;
}
...
/* _________________________
  ...
  |_________________________|
*/
// Return true if Timonel bootloader is detected
bool Timonel::Detected() {
  return status_.signature != 0;
}
...
char Timonel::GetSignature() {
    return status_.signature;
}
/* _________________________
  |                         | 
  |      SetTwiAddress      |
...
uint8_t Timonel::UploadApplication(uint8_t payload[], uint16_t payload_size, const uint16_t start_address) {
    ...
    for (uint16_t i = 0; i < payload_size; i++) {
        ...
        if (page_end++ == (SPM_PAGESIZE - 1)) {
            ...
            if(progressCbPtr) {
                progressCbPtr(100 * i / payload_size);
            }
        }
        esp_task_wdt_reset();
    }
...
uint8_t Timonel::BootloaderInit(void) {
    uint8_t twi_errors = 0;
#if ((defined DEBUG_LEVEL) && (DEBUG_LEVEL >= 1))
    USE_SERIAL.printf_P("[%s] Timonel device 0x%02X * Initialization Step 1 required by features *\r\n", __func__, addr_);
#endif /* DEBUG_LEVEL */
    status_.signature = 0;
...
uint8_t Timonel::QueryStatus(void) {
#if ((defined DEBUG_LEVEL) && (DEBUG_LEVEL >= 1))
    USE_SERIAL.printf_P("[%s] Querying Timonel device 0x%02X to get status ...\r\n", __func__, addr_);
#endif                                           /* DEBUG_LEVEL */
    ...
    if (twi_errors != 0) {
#if ((defined DEBUG_LEVEL) && (DEBUG_LEVEL >= 1))
        USE_SERIAL.printf_P("[%s] Error querying Timonel device 0x%02X to get status (%d) ...\r\n", __func__, addr_, twi_errors);
#endif /* DEBUG_LEVEL */
        return twi_errors;
    } else {
        //if ((twi_reply_arr[CMD_ACK_POS] == ACKTMNLV) && (twi_reply_arr[S_SIGNATURE] == T_SIGNATURE)) {
        if (twi_reply_arr[CMD_ACK_POS] == ACKTMNLV) {
...
uint8_t Timonel::FillSpecialPage(const uint8_t page_type, const uint8_t app_reset_msb, const uint8_t app_reset_lsb) {
    ...
    //delay(DLY_SET_ADDR);
    vTaskDelay(DLY_SET_ADDR / portTICK_PERIOD_MS);
